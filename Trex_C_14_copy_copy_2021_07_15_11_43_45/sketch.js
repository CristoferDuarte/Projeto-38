...
function setup() {
  ...
  fimDeJogo = createSprite(displayWidth / 2,displayHeight-400);
  fimDeJogo.addImage(imgFimDeJogo);
  
  reiniciar = createSprite(displayWidth / 2,displayHeight-360);
  reiniciar.addImage(imgReiniciar);
  
  fimDeJogo.scale = 0.5;
  reiniciar.scale = 0.5;
    
  soloinvisivel = createSprite(displayWidth/2,displayHeight-65,displayWidth,10);
...
function draw() {
  ...
  background(180);
  //exibindo pontuação
  text("Pontuação: "+ pontuacao, displayWidth/2,displayHeight-600);
...
function gerarObstaculos(){
 if (frameCount % 60 === 0){
   var obstaculo = createSprite(400,displayHeight-90,10,40);
...
function gerarNuvens() {
  ...
  if (frameCount % 60 === 0) {
    nuvem = createSprite(600,displayHeight-300,40,10);
